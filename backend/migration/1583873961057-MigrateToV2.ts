...
const sqlValuesPerClassroom = arr.map(([name, numOfComps])=>{
	const oneClassroom : string[] = [];
...
const sqlAllValues = sqlValuesPerClassroom.reduce((acc, val) => acc.concat(val), []).join(", ");
const sqlInsert = `INSERT INTO computers (id, classroomName) VALUES ${sqlAllValues};`;
