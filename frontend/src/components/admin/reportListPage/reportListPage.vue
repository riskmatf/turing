<template>
	...
	<div v-else-if="classroomDataRequestStatus === 'error'" class="text-danger">
		Greska!
	</div>
</template>
		...
		methods: {
			...
			getReports() {
				this.fetchReports({
					...
					this.maxPageCash = maxNumOfPages + 1 || 1
				})
			},
			...
			confirmBreakComputer() {
				...
				const confirmText = this.isCurrentComputerBroken ? 'Vrati računar' : 'Izbaci računar'
				const successMessage = this.isCurrentComputerBroken ? 'Računar je vraćen' : 'Računar je izbačen'
				this.$confirm(text, {
